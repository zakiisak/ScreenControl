...
public class JNAScreenShot
{
    ...
    public static Raster getScreenshotData(Rectangle bounds)
    {
        HDC windowDC = GDI.GetDC(USER.GetDesktopWindow());
        HBITMAP outputBitmap = GDI.CreateCompatibleBitmap(windowDC, bounds.width, bounds.height);
        try
        {
            ...
            try
            {
                ...
                if (ok)
                {
                    ...
                    bih.biHeight = -Math.abs(bih.biHeight);
                    bi.bmiHeader.biCompression = 0;
                    return getRaster(blitDC, outputBitmap, bi);
                }
                else
                {
                    return null;
    ...
    private static Raster getRaster(HDC blitDC, HBITMAP outputBitmap, BITMAPINFO bi)
    {
        BITMAPINFOHEADER bih = bi.bmiHeader;
